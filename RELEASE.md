# Release Process — GroupDocs.Merger.Mcp
...
1. **Bump the package version** in [build/dependencies.props](build/dependencies.props):
   ...
   ```xml
   <GroupDocsMergerMcp>{NEW_VERSION}</GroupDocsMergerMcp>
   ```

2. **Bump both `.mcp/server.json` versions** — [src/GroupDocs.Merger.Mcp/.mcp/server.json](src/GroupDocs.Merger.Mcp/.mcp/server.json) has **two** `version` fields that must both match:
...
3. **Update pinned versions in `README.md`** — search for the old version and replace every occurrence (`dnx GroupDocs.Merger.Mcp@…`, Claude Desktop config, VS Code `mcp.json`). Typically 4 places.
...
5. *(Optional, rare)* Bump external dependency versions in the same props file — `GroupDocsMcpCore`, `GroupDocsMerger`, `MicrosoftExtensionsHosting`, `ModelContextProtocol`, `MicrosoftSourceLinkGithub`.

6. Commit + push:

   ```bash
   git add build/dependencies.props src/GroupDocs.Merger.Mcp/.mcp/server.json README.md changelog/NNN-*.md
...
dotnet test src/GroupDocs.Merger.Mcp.sln -c Release       # runs tests
docker build -f docker/Dockerfile -t merger-net-mcp:test .    # Dockerfile sanity check
```
...
4. Push `ghcr.io/groupdocs-merger/merger-net-mcp:{NEW_VERSION}` + `:latest`.
5. Push `docker.io/groupdocs/merger-net-mcp:{NEW_VERSION}` + `:latest`.

**`publish_prod.yml → publish_mcp_registry` job** (runs on every release):

1. Runs after `publish_prod` succeeds.
2. Uses GitHub OIDC to authenticate with the MCP Registry — namespace `io.github.groupdocs-merger/*` is auto-verified because the repo lives under `github.com/groupdocs-merger/*`.
3. Publishes `src/GroupDocs.Merger.Mcp/.mcp/server.json` to `https://registry.modelcontextprotocol.io`.
...
- [ ] `ghcr.io/groupdocs-merger/merger-net-mcp:{NEW_VERSION}` pullable
- [ ] `docker.io/groupdocs/merger-net-mcp:{NEW_VERSION}` pullable
- [ ] Smoke test from a clean machine: `dnx GroupDocs.Merger.Mcp@{NEW_VERSION} --yes`
- [ ] MCP Registry: `https://registry.modelcontextprotocol.io/v0/servers/io.github.groupdocs-merger%2Fgroupdocs-merger-mcp/versions/latest` returns 200 with `version` = `{NEW_VERSION}` and `_meta.io.modelcontextprotocol.registry/official.isLatest` = `true` (note: the slash in the server name must be URL-encoded as `%2F`, and the route requires the `/versions/latest` suffix — there is no bare `/v0/servers/{name}` endpoint)
...
| Secret | Purpose |
|---|---|
| `NUGET_API_KEY_PROD` | NuGet.org API key scoped to `GroupDocs.Merger.Mcp` |
...
| `DOCKERHUB_TOKEN` | Docker Hub access token scoped to `groupdocs/merger-net-mcp` |
...
The `publish_mcp_registry` job authenticates via GitHub OIDC. The Registry auto-verifies the `io.github.groupdocs-merger/*` namespace because the workflow runs inside `github.com/groupdocs-merger/*` — no DNS records, no manual tokens, no opt-in variable required. First-time publish claims the namespace automatically; subsequent releases update it.

If you ever fork the repo to a different org (e.g. `github.com/someone-else/*`), the Registry publish will fail until either the `server.json` `name` field is updated to match the new org's namespace or DNS verification is added — but for the canonical `groupdocs-merger` org, it just works.
