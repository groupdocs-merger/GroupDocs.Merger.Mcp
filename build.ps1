...
Assert-ServerJsonVersionMatchesDependencies `
    ...
    -ServerJsonPath ".\src\GroupDocs.Merger.Mcp\.mcp\server.json" `
    -VersionPropertyName "GroupDocsMergerMcp"

exec { & dotnet restore src\GroupDocs.Merger.Mcp.sln }

$isProd = $env:BUILD_TYPE -eq "PROD"

if ($isProd) {
    Write-Host "build: PROD build - stable version (no suffix)"
    exec { & dotnet build src\GroupDocs.Merger.Mcp.sln -c Release --verbosity quiet --nologo }
} else {
    $commitHash = $(git rev-parse --short HEAD)
    $buildSuffix = "local-$commitHash"
    Write-Host "build: DEV build - version suffix is $buildSuffix"
    exec { & dotnet build src\GroupDocs.Merger.Mcp.sln -c Release --version-suffix=$buildSuffix --verbosity quiet --nologo }
}
...
exec { & dotnet pack .\src\GroupDocs.Merger.Mcp\GroupDocs.Merger.Mcp.csproj @packArgs }
